...
@section('content')
        ...
        <div class="card mb-4" style="margin: 0px 150px 100px 150px;">
            <form  action="{{ route('store_testimony') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control {{ $errors->has('company') ? ' border-danger' : '' }}" name="company" value="{{ old('company') }} ">
                        @error('company')
                            <span class="text-danger"> {{$message}} </span>
                        @enderror
                    </div>
